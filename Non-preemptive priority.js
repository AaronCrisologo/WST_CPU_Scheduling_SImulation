...
// Select the next process to run based on priority (higher priority first)
function selectNextProcess(availableProcesses) {
    // Sort available processes by priority, then by arrival time
    availableProcesses.sort((a, b) => {
        if (processes[a].priority === processes[b].priority) {
            return processes[a].arrivalTime - processes[b].arrivalTime; // If priorities are equal, sort by arrival time
        }
        return processes[a].priority - processes[b].priority; // Lower number indicates higher priority
    });
